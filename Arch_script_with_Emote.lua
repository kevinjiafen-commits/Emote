...
-- ============================================================================
-- EMOTE CORE (extracted from the supplied pasted-content Emote implementation)
-- ============================================================================
-- This section intentionally keeps the original Emote playback logic separate
-- from the Arch UI so it can coexist with the existing library safely.

local EmotePRESET_ORDER = {
...
local EmoteHeartbeatConnection = nil
local EmoteCharacterConnection = nil

-- @param char Instance|nil  Character/model whose Humanoid animator is needed.
-- @return Animator|nil
local function EmoteGetAnimator(char)
    ...
    if not anim then
        ...
        if ok then anim = created end
    end
    return anim
end

-- @param id string Animation asset id (digits or an rbxassetid id string).
-- @return Animation|nil
local function EmoteResolveAnim(id)
    if type(id) ~= "string" or id == "" then return nil end

    local cleanId = id:match("(%d+)") or id
    ...
    if ok and type(res) == "table" and #res > 0 then
        for _, o in ipairs(res) do
            if o and o:IsA("Animation") and o.AnimationId ~= "" then
                return o
            end
        end
        for _, o in ipairs(res) do
    ...
    a.AnimationId = "rbxassetid://" .. cleanId
    return a
end

-- @param char Instance|nil Character/model to play on.
-- @param id string Animation id.
-- @param spd number Playback speed.
-- @param loop boolean Whether the track loops.
-- @return AnimationTrack|nil
local function EmoteLoadPlay(char, id, spd, loop)
    ...
    local ok, track = pcall(function()
        return animator:LoadAnimation(obj)
    end)
    ...
    pcall(function()
        ...
        track:Play(0.1, 1, tonumber(spd) or 1)
    end)
...
local function EmoteStopAll()
    for _, t in ipairs(EmoteActiveTracks) do
        pcall(function()
            if t then
                t:Stop(0)
                t:Destroy()
...
local function EmotePlay()
    ...
    if not EmoteState.enabled or EmoteState.animationId == "" then
        return
    end

    local char = LocalPlayer and LocalPlayer.Character
    if char then
        local track = EmoteLoadPlay(char, EmoteState.animationId, EmoteState.speed, EmoteState.loop)
        if track then
            table.insert(EmoteActiveTracks, track)
        end
    end
    ...
    if live and LocalPlayer then
        ...
        if sc then
            local track = EmoteLoadPlay(sc, EmoteState.animationId, EmoteState.speed, EmoteState.loop)
            if track then
                table.insert(EmoteActiveTracks, track)
            end
        end
...
if EmoteHeartbeatConnection then
    EmoteHeartbeatConnection:Disconnect()
    EmoteHeartbeatConnection = nil
end

EmoteHeartbeatConnection = RunService.Heartbeat:Connect(function()
    ...
    local speed = tonumber(EmoteState.speed) or 1
    for i = #EmoteActiveTracks, 1, -1 do
        local track = EmoteActiveTracks[i]
        local alive = false
        pcall(function()
        ...
        if not alive then
            table.remove(EmoteActiveTracks, i)
        end
    end
end)

if EmoteCharacterConnection then
    EmoteCharacterConnection:Disconnect()
    EmoteCharacterConnection = nil
end

EmoteCharacterConnection = LocalPlayer.CharacterAdded:Connect(function()
    task.wait(0.5)
...
local function EmoteSetCustomId(id)
    ...
    EmoteState.animationId = cleanId
    EmotePlay()
    return true
end
...
-- @return boolean success
local function EmoteSetPreset(name)
    if type(name) ~= "string" then return false end
    local id = EmoteAnimPresets[name]
...
-- ============================================================================
-- EMOTE TAB
-- Placed directly after the Config/Settings tab in the sidebar.
-- ============================================================================
local EmoteTab = Library:Tab("Emote", 11888734334)

local EmotePresetGroup = EmoteTab:Group("Presets")
local EmotePresetButtons = {}

for _, presetName in ipairs(EmotePRESET_ORDER) do
    local presetButton
    presetButton = EmotePresetGroup:Button({
        Name = presetName,
        Tooltip = "Play " .. presetName .. " (click the active preset again to stop)",
        Callback = function()
            local wasSame = EmoteState.enabled and EmoteState.presetName == presetName
            EmoteSetPreset(presetName)

            for name, btn in pairs(EmotePresetButtons) do
                if btn and btn.Parent then
                    if name == presetName and not wasSame then
    ...
    EmotePresetButtons[presetName] = presetButton
end

local EmoteCustomGroup = EmoteTab:Group("Custom ID")
local EmoteCustomId = ""
EmoteCustomGroup:Textbox({
    Name = "Animation Asset ID",
    Placeholder = "1234567890",
    Callback = function(text)
        EmoteCustomId = tostring(text or "")
    end,
    Tooltip = "Enter a Roblox animation asset ID",
})
EmoteCustomGroup:Button({
    Name = "Play Custom ID",
    Variant = "Primary",
    Callback = function()
        if EmoteSetCustomId(EmoteCustomId) then
            for _, btn in pairs(EmotePresetButtons) do
                if btn and btn.Parent then
                    btn.BackgroundColor3 = CFG.SecondaryColor
                    btn.TextColor3 = CFG.TextDark
                end
...
local EmoteSettingsGroup = EmoteTab:Group("Playback")
EmoteSettingsGroup:Slider({
    Name = "Speed",
    Min = 1,
    Max = 500,
    ...
    Callback = function(v)
        local n = tonumber(v)
        if n then
            EmoteState.speed = math.clamp(n, 1, 500)
        end
    end,
    Tooltip = "Playback speed",
})
EmoteSettingsGroup:Toggle({
    Name = "Loop",
    Default = EmoteState.loop,
    Callback = function(v)
        EmoteState.loop = v == true
...
EmoteSettingsGroup:Toggle({
    ...
    Callback = function(v)
        EmoteState.spawnProof = v == true
    end,
    Tooltip = "Automatically replay the selected emote after respawn",
})
EmoteSettingsGroup:Button({
    Name = "Stop Emote",
    Variant = "Danger",
    Callback = function()
        EmoteStop()
