...
-- ============================================================
-- Emote module
-- Ported from the supplied standalone EmoteUI logic.
-- The standalone panel is intentionally replaced by Arch's
-- existing Tab/Group UI, while all playback/state functions
-- and preset data are retained.
-- ============================================================
...
local function resolveAnim(id)
    ...
    if ok and type(res) == "table" and #res > 0 then
        for _, o in ipairs(res) do
            if typeof(o) == "Instance" and o:IsA("Animation") and o.AnimationId ~= "" then
                return o
            end
        end
...
-- @param char Instance|nil Character model
-- @param id string Animation asset id
-- @param spd number Playback speed
-- @param looped boolean Whether the animation loops
-- @return AnimationTrack|nil
local function loadPlay(char, id, spd, looped)
    if not char or type(id) ~= "string" or id == "" then
        return nil
    end
...
local function playEmote()
    ...
    if not S.emoteEnabled or type(S.emoteAnimationId) ~= "string" or S.emoteAnimationId == "" then
        return
    end

    local char = LocalPlayer.Character
    if char then
        local t = loadPlay(char, S.emoteAnimationId, S.emoteSpeed, S.emoteLoop)
        if t then
            table.insert(activeTracks, t)
        end
    end
    ...
    if Live then
        ...
        if sc then
            local t = loadPlay(sc, S.emoteAnimationId, S.emoteSpeed, S.emoteLoop)
            if t then
                table.insert(activeTracks, t)
            end
        end
...
RunService.Heartbeat:Connect(function()
    if not S.emoteEnabled then
        return
    end

    if #activeTracks == 0 and S.emoteAnimationId ~= "" then
        playEmote()
    end

    for _, t in ipairs(activeTracks) do
        pcall(function()
            t:AdjustSpeed(tonumber(S.emoteSpeed) or 1)
        end)
    end
end)
...
LocalPlayer.CharacterAdded:Connect(function()
    task.wait(0.5)
    if S.emoteEnabled and S.emoteSpawnProof then
        playEmote()
    end
end)
