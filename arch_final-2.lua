...
-- ============================================================
-- EMOTE SYSTEM (extracted from Eclipse Emote UI)
-- ============================================================
...
local function emoteResolveAnim(id)
    ...
    if ok and type(objects) == "table" then
        for _, obj in ipairs(objects) do
            if obj:IsA("Animation") and obj.AnimationId ~= "" then return obj end
        end
        for _, obj in ipairs(objects) do
            for _, child in ipairs(obj:GetDescendants()) do
                if child:IsA("Animation") and child.AnimationId ~= "" then return child end
            end
        end
    end
...
local function emoteLoadPlay(char, id, speed, looped)
    ...
    pcall(function() track:Play(0.1, 1, tonumber(speed) or 1) end)
    return track
end
...
-- Heartbeat: keep emote alive and sync speed changes
RunService.Heartbeat:Connect(function()
    ...
    for _, track in ipairs(emoteActiveTracks) do
        if track then
            pcall(function() track:AdjustSpeed(emoteState.speed) end)
        end
    end
end)

-- SpawnProof: re-fire emote on respawn
-- (hooked into the existing CharacterAdded at bottom of script)
-- We register a separate connection here so it doesn't conflict
LocalPlayer.CharacterAdded:Connect(function()
    task.wait(0.5)
...
-- ============================================================
-- EMOTE UI — Settings tab right column (EmoteGroup)
-- ============================================================

local EmoteGroup = ConfigTab:Group("Emote")

local emoteActivePreset   = nil
local emotePresetButtons  = {}

local function emoteClearPreset()
    if emoteActivePreset and emotePresetButtons[emoteActivePreset] then
        local btn = emotePresetButtons[emoteActivePreset]
        pcall(function()
    ...
    emoteActivePreset = nil
end

for _, emoteName in ipairs(EMOTE_PRESET_ORDER) do
    local btn = EmoteGroup:Button({
        Name = emoteName,
        Callback = function()
            if emoteState.enabled and emoteState.presetName == emoteName then
            ...
            emoteState.presetName  = emoteName
            emoteState.animationId = emoteAnimPresets[emoteName] or ""
            emoteActivePreset              = emoteName
            emotePresetButtons[emoteName]  = btn
            pcall(function()
                TweenService:Create(btn, TweenInfo.new(0.15), {
                    BackgroundColor3 = CFG.SecondaryColor,
            ...
            emotePlay()
            Library:Notify("Playing: " .. emoteName, "success")
        end,
    })
    emotePresetButtons[emoteName] = btn
end

local emoteCustomIDBox = EmoteGroup:Textbox({
    Name        = "Custom Anim ID",
