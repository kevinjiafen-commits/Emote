...
-- ============================================================
-- Auto Collect Boosters FFA
-- Integrated into the existing Arch UI.
-- No second ScreenGui is created.
-- ============================================================
...
local function BoosterGetCharacter()
...
local function BoosterGetRoot(character)
...
local function BoosterHideObject(obj)
...
local function BoosterIsRelevantDrop(obj)
    if not obj or not obj.Parent or obj.Name ~= "_drop" then
        return false
    end
    ...
    pcall(function()
        hasAmmo = BoosterSettings.AutoAmmo and obj:FindFirstChild("Ammo") ~= nil
        hasHealth = BoosterSettings.AutoHealth and obj:FindFirstChild("Health") ~= nil
    end)

    return hasAmmo or hasHealth
end

local function BoosterProcessDrop(obj, rootPosition, bounce)
    if not BoosterIsRelevantDrop(obj) then
        return
    end

    pcall(function()
        obj.Anchored = true
        obj.CFrame = CFrame.new(rootPosition + Vector3.new(0, bounce, 0))
        obj.Transparency = 1
    end)

    BoosterHideObject(obj)
end

local function BoosterCollectStep(dt)
    local character = BoosterGetCharacter()
    local root = BoosterGetRoot(character)
    if not root then
        return
    end

    BoosterState.BounceTime += (tonumber(dt) or 0) * 8
    local bounce = math.sin(BoosterState.BounceTime) * 4
    ...
    for _, obj in ipairs(children) do
        BoosterProcessDrop(obj, rootPosition, bounce)
    end
end

local function BoosterStartCollector()
    if BoosterState.Connection then
        return
    end

    if not RunService or not RunService.RenderStepped then
        return
    end

    BoosterState.Connection = RunService.RenderStepped:Connect(function(dt)
        pcall(BoosterCollectStep, dt)
    end)
end

local function BoosterStopCollector()
    if BoosterState.Connection then
        pcall(function()
...
local function BoosterTryRespawn()
    if not BoosterSettings.AutoRespawn then
        return
    end

    local keypressFn = rawget(getfenv and getfenv() or _G, "keypress")
    local keyreleaseFn = rawget(getfenv and getfenv() or _G, "keyrelease")
...
local function BoosterBindCharacter(character)
    ...
    local old = BoosterState.DeathConnections[character]
    if old then
        pcall(function()
            old:Disconnect()
        end)
    ...
    BoosterState.DeathConnections[character] = humanoid.Died:Connect(function()
        BoosterTryRespawn()
    end)
end

local function BoosterStartRespawnWatcher()
    local lp = Players and Players.LocalPlayer
    if not lp then
        return
    end

    if BoosterState.CharacterConnection then
        pcall(function()
            BoosterState.CharacterConnection:Disconnect()
        end)
        BoosterState.CharacterConnection = nil
    end

    local current = lp.Character
    if current then
        task.spawn(function()
            BoosterBindCharacter(current)
        end)
    end

    BoosterState.CharacterConnection = lp.CharacterAdded:Connect(function(character)
        task.spawn(function()
            BoosterBindCharacter(character)
        end)
    end)
end

local function BoosterSetAutoHealth(enabled)
    BoosterSettings.AutoHealth = enabled == true
end

local function BoosterSetAutoAmmo(enabled)
    BoosterSettings.AutoAmmo = enabled == true
end

local function BoosterSetAutoRespawn(enabled)
    BoosterSettings.AutoRespawn = enabled == true
end

BoosterStartCollector()
BoosterStartRespawnWatcher()
...
local Boosters = Library:Tab("Boosters", 3926305904)
local BoostersGroup = Boosters:Group("Auto Collect Boosters FFA")

BoostersGroup:Toggle({
    Name = "Auto Health",
    Default = BoosterSettings.AutoHealth,
    Tooltip = "Automatically collect Health drops.",
    Callback = function(v)
        BoosterSetAutoHealth(v)
    end
})

BoostersGroup:Toggle({
    Name = "Auto Ammo",
    Default = BoosterSettings.AutoAmmo,
    Tooltip = "Automatically collect Ammo drops.",
    Callback = function(v)
        BoosterSetAutoAmmo(v)
    end
})

BoostersGroup:Toggle({
    Name = "Auto Respawn",
    Default = BoosterSettings.AutoRespawn,
    Tooltip = "Attempts automatic respawn using the available input API.",
    Callback = function(v)
        BoosterSetAutoRespawn(v)
    end
})
