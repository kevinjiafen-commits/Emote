...
local function encodeValue(v)
    ...
    elseif type(v) == "function" or typeof(v) == "RBXScriptConnection" or typeof(v) == "Instance" or typeof(v) == "userdata" then
        return nil
    else
        return v
    end
...
local function decodeValue(v)
    if type(v) == "table" then
        ...
        elseif v.__type == "Enum" then
            ...
            if enumType then
                local ok, item = pcall(function() return enumType[v.name] end)
...
local function saveConfig()
    local data = { _S = {}, _Rage = {}, _autoLoad = autoLoadConfig }
    for k, v in pairs(S) do
        local enc = encodeValue(v)
        if enc ~= nil then data._S[k] = enc end
    end
    if RageSettings then
        for k, v in pairs(RageSettings) do
            local enc = encodeValue(v)
            if enc ~= nil then data._Rage[k] = enc end
        end
    end
...
local function loadConfig(silent)
    ...
    local ok1, exists = pcall(isfile, configName)
    if not ok1 or not exists then
        if not silent then Library:Notify("配置文件不存在", "warning") end
        return
    end
    local ok2, raw = pcall(readfile, configName)
    if not ok2 or not raw or raw == "" then
        if not silent then Library:Notify("读取失败", "warning") end
        return
    end
    ...
    local sData = data._S or data
    local rData = data._Rage or {}

    -- Restore S: merge into existing, never replace the table reference
    for k, v in pairs(sData) do
        -- Skip internal keys from new format stored at top level
        if k ~= "_Rage" and k ~= "_autoLoad" then
    ...
    -- Restore RageSettings: merge into existing Settings table inside RageCore
    if RageSettings then
        for k, v in pairs(rData) do
            local decoded = decodeValue(v)
            if decoded ~= nil then
                RageSettings[k] = decoded
            end
        end
    end

    -- Restore autoLoadConfig flag if present
    if type(data._autoLoad) == "boolean" then
        autoLoadConfig = data._autoLoad
    end
...
-- Correct boot sequence: load config first, then check flag from the loaded data
loadConfig(true)
-- autoLoadConfig is now whatever was saved; if it was true, settings are already applied
