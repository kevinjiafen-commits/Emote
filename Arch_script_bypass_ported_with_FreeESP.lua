...
-- ============================================================
-- FREE-SCRIPT ESP (Drawing-based Box + Chams + Text labels)
-- Ported from arch script free — lives in the RIGHT column
-- of the Visuals tab, separate from the mobile ESP above.
-- ============================================================
local freeESP = {
    enabled      = false,
    boxEnabled   = false,
...
local freeESP_boxes  = {}   -- Drawing.Square per player
local freeESP_chams  = {}   -- Highlight per player
local freeESP_texts  = {}   -- Drawing.Text per player

local function freeESP_cleanup(player)
    if freeESP_boxes[player] then
        pcall(function() freeESP_boxes[player]:Remove() end)
        freeESP_boxes[player] = nil
    end
    if freeESP_chams[player] then
        pcall(function() freeESP_chams[player]:Destroy() end)
        freeESP_chams[player] = nil
    end
    if freeESP_texts[player] then
        pcall(function() freeESP_texts[player]:Remove() end)
        freeESP_texts[player] = nil
    end
end
...
local function freeESP_create(player)
    ...
    text.Transparency = 1
    text.Font         = 2
    freeESP_texts[player] = text
end

local function freeESP_setupPlayer(player)
    if player == Players.LocalPlayer then return end
...
for _, p in ipairs(Players:GetPlayers()) do freeESP_setupPlayer(p) end
Players.PlayerAdded:Connect(freeESP_setupPlayer)
Players.PlayerRemoving:Connect(freeESP_cleanup)

local freeESP_conn
local function freeESP_start()
    if freeESP_conn then return end
    freeESP_conn = RunService.Heartbeat:Connect(function()
        ...
        for _, player in ipairs(Players:GetPlayers()) do
            ...
            if not hum or not root then freeESP_cleanup(player); continue end

            if not freeESP_boxes[player] then freeESP_create(player) end

            local cam = workspace.CurrentCamera
            local rootPos, onScreen = cam:WorldToViewportPoint(root.Position)

            -- ── Text label ──────────────────────────────────────
            if onScreen and freeESP_texts[player] then
                local str = ""
                if freeESP.showName     then str = str .. player.Name .. "\n" end
                if freeESP.showDistance and localRoot then
                    str = str .. math.floor((localRoot.Position - root.Position).Magnitude) .. " studs\n"
                end
                if freeESP.showHealth then
                    str = str .. math.floor(hum.Health) .. "/" .. hum.MaxHealth .. "\n"
                end
                ...
                freeESP_texts[player].Text     = str
                freeESP_texts[player].Position = Vector2.new(rootPos.X, rootPos.Y - 50)
                freeESP_texts[player].Visible  = true
            elseif freeESP_texts[player] then
                freeESP_texts[player].Visible = false
            end

            -- ── Box ESP ─────────────────────────────────────────
            if freeESP.boxEnabled and freeESP_boxes[player] and onScreen then
                ...
                for _, part in ipairs(player.Character:GetDescendants()) do
                    if part:IsA("BasePart") then
                        ...
                        for _, c in ipairs(corners) do
                            local sp = cam2:WorldToViewportPoint(c.Position)
                            minX = math.min(minX, sp.X); maxX = math.max(maxX, sp.X)
                            minY = math.min(minY, sp.Y); maxY = math.max(maxY, sp.Y)
                        end
                    end
                end
                freeESP_boxes[player].Size     = Vector2.new(maxX - minX, maxY - minY)
                freeESP_boxes[player].Position = Vector2.new(minX, minY)
                freeESP_boxes[player].Visible  = true
            elseif freeESP_boxes[player] then
                freeESP_boxes[player].Visible = false
            end

            -- ── Chams ────────────────────────────────────────────
            if freeESP_chams[player] then
                if freeESP.chamsEnabled then
                    if freeESP_chams[player].Adornee ~= player.Character then
                        freeESP_chams[player].Adornee = player.Character
                    end
                    freeESP_chams[player].Enabled = true
                else
                    freeESP_chams[player].Enabled = false
                end
            end
        end
    end)
...
local function freeESP_stop()
    if freeESP_conn then
        freeESP_conn:Disconnect()
        freeESP_conn = nil
    end
    for player in pairs(freeESP_boxes) do freeESP_cleanup(player) end
end

-- UI — forced onto right column by calling Group twice (alternating left/right)
-- The mobile ESPGroup above consumed one left slot; this Group goes right.
local FreeESPGroup = Visuals:Group("Player ESP (Free)")
...
FreeESPGroup:Toggle({
    ...
    Callback = function(v)
        freeESP.enabled = v
        if v then freeESP_start()
        else
            freeESP.boxEnabled   = false
            freeESP.chamsEnabled = false
            freeESP_stop()
        end
    end
})
...
FreeESPGroup:Toggle({
    Name    = "Box ESP",
    Default = false,
    Callback = function(v) freeESP.boxEnabled = v end
})

FreeESPGroup:Toggle({
    Name    = "Chams",
    Default = false,
    Callback = function(v) freeESP.chamsEnabled = v end
})

FreeESPGroup:Toggle({
    Name    = "Name",
    Default = true,
    Callback = function(v) freeESP.showName = v end
})

FreeESPGroup:Toggle({
    Name    = "Distance",
    Default = true,
    Callback = function(v) freeESP.showDistance = v end
})

FreeESPGroup:Toggle({
    Name    = "Health",
    Default = true,
    Callback = function(v) freeESP.showHealth = v end
})

FreeESPGroup:Toggle({
    Name    = "Weapon",
    Default = true,
    Callback = function(v) freeESP.showWeapon = v end
})
