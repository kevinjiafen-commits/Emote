...
--==================================================
-- EMOTE DATA / STATE (from Eclipse-style Emote UI)
--==================================================
...
local function emote_play()
    ...
    if live then
        local sc = live:FindFirstChild(Players.LocalPlayer.Name)
        if sc then
            local track = emote_loadPlay(
                sc,
                emoteState.animationId,
...
-- Emote heartbeat: re-trigger if tracks disappear, adjust speed live
game:GetService("RunService").Heartbeat:Connect(function()
    if not emoteState.enabled then return end
    if #activeTracks == 0 and emoteState.animationId ~= "" then
        emote_play()
        return
    end
    for _, track in ipairs(activeTracks) do
...
-- Emote spawn-proof: re-play after character respawn
...
--==================================================
-- EMOTE UI (Settings 分页 / 右栏)
--==================================================

local EmoteGroup = ConfigTab:Group("Emote Presets")

local activePreset = nil
local presetButtons = {}

local function emoteUI_clearPreset()
...
for _, name in ipairs(PRESET_ORDER) do
    local button = EmoteGroup:Button({
        Name = name,
        Callback = function()
            -- 再次点击同一个 preset → 停止
            ...
            emoteState.animationId = animPresets[name] or ""
            ...
            emote_play()
            Library:Notify("Emote: " .. name, "success")
        end,
    })
    presetButtons[name] = button
end
...
-- 存储 textbox 引用用于 Get()
local emoteCustomIDBox = {text = ""}
local emoteCustomFrame = Create("Frame", {
    Parent = nil, -- 将由 Textbox 内部创建
    Size = UDim2.new(1, 0, 0, 35),
    BackgroundTransparency = 1,
})

-- 用原生方式创建 textbox，获得引用
EmoteCustomGroup:Textbox({
    Name = "Animation ID",
    Placeholder = "Asset ID...",
    Callback = function(val)
        emoteCustomIDBox.text = val
    end,
})
...
EmoteCustomGroup:Button({
    ...
    Variant = "Primary",
    Callback = function()
        local raw = emoteCustomIDBox.text
...
EmoteOptionsGroup:Toggle({
    ...
    Tooltip = "Loop the emote animation",
    Callback = function(value)
...
EmoteOptionsGroup:Toggle({
    ...
    Tooltip = "Re-play emote after respawn",
    Callback = function(value)
