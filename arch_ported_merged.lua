...
-- ============================================
-- FREE ESP (Drawing-based) — RIGHT COLUMN
-- Ported from arch script free; uses Drawing API
-- + Highlight chams. Runs alongside Mobile ESP.
-- ============================================
local freeESP = {
    enabled    = false,
    box        = true,
...
local function freeESP_cleanup(player)
    if freeEspBoxes[player] then
        pcall(function() freeEspBoxes[player]:Remove() end)
        freeEspBoxes[player] = nil
    end
    if freeEspChams[player] then
        pcall(function() freeEspChams[player]:Destroy() end)
        freeEspChams[player] = nil
    end
    if freeEspTexts[player] then
        pcall(function() freeEspTexts[player]:Remove() end)
        freeEspTexts[player] = nil
    end
end

local function freeESP_create(player)
    if player == LocalPlayer then return end
    if not player.Character then return end
    freeESP_cleanup(player)
    ...
    box.Filled       = false
    box.Transparency = 1
    freeEspBoxes[player] = box

    local cham = Instance.new("Highlight")
    cham.Name               = "FreeESP_Cham_" .. player.Name
    ...
    text.Transparency = 1
    text.Font         = 2
    freeEspTexts[player] = text
end

local function freeESP_setup(player)
    if player == LocalPlayer then return end
    player.CharacterAdded:Connect(function()
        task.wait(0.1)
        if freeESP.enabled then freeESP_create(player) end
    end)
    ...
    if player.Character then
        task.wait(0.1)
        freeESP_create(player)
    end
end

for _, p in ipairs(Players:GetPlayers()) do freeESP_setup(p) end
Players.PlayerAdded:Connect(freeESP_setup)
Players.PlayerRemoving:Connect(freeESP_cleanup)

local freeEspConn
local function toggleFreeESP(on)
    freeESP.enabled = on
    if on then
        if freeEspConn then freeEspConn:Disconnect() end
        freeEspConn = RunService.Heartbeat:Connect(function()
            if not freeESP.enabled then return end
            local cam = workspace.CurrentCamera
            for _, player in ipairs(Players:GetPlayers()) do
                if player == LocalPlayer then continue end
                ...
                if not humanoid or not root then freeESP_cleanup(player); continue end
                if not freeEspBoxes[player] then freeESP_create(player) end

                local rootPos, onScreen = cam:WorldToViewportPoint(root.Position)

                -- Text (name / dist / health / weapon)
                if onScreen and freeEspTexts[player] then
                    local myRoot = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
                    local str = ""
                    if freeESP.showName   then str = str .. player.Name .. "\n" end
                    if freeESP.showDist and myRoot then
                        str = str .. math.floor((myRoot.Position - root.Position).Magnitude) .. " studs\n"
                    end
                    if freeESP.showHealth then
                        str = str .. math.floor(humanoid.Health) .. "/" .. humanoid.MaxHealth .. "\n"
                    end
                    if freeESP.showWeapon then
                        local tool = player.Character:FindFirstChildOfClass("Tool")
                            or (player.Backpack and player.Backpack:FindFirstChildOfClass("Tool"))
                        if tool then str = str .. tool.Name end
                    end
                    freeEspTexts[player].Text     = str
                    freeEspTexts[player].Position = Vector2.new(rootPos.X, rootPos.Y - 50)
                    freeEspTexts[player].Visible  = true
                elseif freeEspTexts[player] then
                    freeEspTexts[player].Visible = false
                end

                -- Box ESP
                if freeESP.box and onScreen and freeEspBoxes[player] then
                    ...
                    for _, part in ipairs(player.Character:GetDescendants()) do
                        if part:IsA("BasePart") then
                            ...
                            for _, offset in ipairs({
                                ...
                                local sv = cam:WorldToViewportPoint(corner.Position)
                                minX = math.min(minX, sv.X); maxX = math.max(maxX, sv.X)
                                minY = math.min(minY, sv.Y); maxY = math.max(maxY, sv.Y)
                            end
                        end
                    end
                    freeEspBoxes[player].Size     = Vector2.new(maxX - minX, maxY - minY)
                    freeEspBoxes[player].Position = Vector2.new(minX, minY)
                    freeEspBoxes[player].Visible  = true
                elseif freeEspBoxes[player] then
                    freeEspBoxes[player].Visible = false
                end

                -- Chams
                if freeEspChams[player] then
                    if freeESP.chams then
                        if freeEspChams[player].Adornee ~= player.Character then
                            freeEspChams[player].Adornee = player.Character
                        end
                        freeEspChams[player].Enabled = true
                    else
                        freeEspChams[player].Enabled = false
                    end
                end
            end
        end)
    else
        if freeEspConn then freeEspConn:Disconnect(); freeEspConn = nil end
        for _, player in ipairs(Players:GetPlayers()) do
            if freeEspBoxes[player]  then freeEspBoxes[player].Visible  = false end
            if freeEspTexts[player]  then freeEspTexts[player].Visible  = false end
            if freeEspChams[player]  then freeEspChams[player].Enabled  = false end
        end
    end
end

-- UI: right column (second Group call on Visuals tab = RightCol)
local FreeESPGroup = Visuals:Group("Player ESP (Drawing)")
FreeESPGroup:Toggle({Name = "Enable", Callback = function(v) toggleFreeESP(v) end})
FreeESPGroup:Toggle({Name = "Box ESP", Default = true, Callback = function(v) freeESP.box = v end})
FreeESPGroup:Toggle({Name = "Chams", Callback = function(v) freeESP.chams = v end})
FreeESPGroup:Toggle({Name = "Name", Default = true, Callback = function(v) freeESP.showName = v end})
FreeESPGroup:Toggle({Name = "Distance", Default = true, Callback = function(v) freeESP.showDist = v end})
FreeESPGroup:Toggle({Name = "Health", Default = true, Callback = function(v) freeESP.showHealth = v end})
FreeESPGroup:Toggle({Name = "Weapon", Default = true, Callback = function(v) freeESP.showWeapon = v end})
...
LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    if freeESP.enabled then toggleFreeESP(true) end
